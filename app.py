...
engine = create_engine('postgresql://postgres:mftUwFbc7aZl2kG5kPdx@nps-demo-instance.clbtdovfplge.sa-east-1.rds.amazonaws.com/strategy')
...
def filter_df(df, exch, lev, s_date, e_date):
    
    f1 = df['exchange'] == exch
    f2 = df['margin'] == int(lev)
    f3 = (df['Entry time'] >= s_date) & (df['Entry time'] <= e_date) 
    ftot = f1 & f2 & f3
    df_ret = df.loc[ftot].copy()
    #df_ret = df.loc[ftot]
    return df_ret
...
def update_barplot(exchange, leverage, start_date, end_date):
   dff = filter_df(df, exchange, leverage, start_date, end_date)
